...
# 1. Blended method implementation
# Selected models
...
hazplot_Pem_blended <- plot_blended_hazard(
  h_Pem_blended, h_Pem_selected, h_Scha_selected, 
  rebased_time, t1, t2, 'Pembrolizumab')

hazplot_Ipi_blended <- plot_blended_hazard(
  h_Ipi_blended, h_Ipi_selected, h_Scha_selected, 
  rebased_time, t1, t2, 'Ipilimumab')

hazplot_blended <- ggarrange(hazplot_Pem_blended, hazplot_Ipi_blended)

print(hazplot_blended)

# 2. Comparison of extrapolated hazard and survival curves (blended method vs updated 7-year data vs TA366 base case)
# 2.1. Reproduce TA366 base case (the best standard parametric model rebased at 12 months - rebased Gompertz)
...
hazplot_TA366_selection <- plot_rebased_hazard(
  models_to_plot = models, 
  haz = haz_Scha, 
  rebased_time = rebased_time_TA366, 
  title = 'Schadendorf (2015) - Hazard of Parametric Models Rebased at 12 Months')

survplot_TA366_selection <- plot_rebased_survival(
  models_to_plot = models, 
  km = km_Scha, 
  rebased_time = rebased_time_TA366, 
...
aic_bic_summary_Scha_rebased_TA366 <- data.frame(
  AIC = sapply(m_Scha_rebased_TA366$models, AIC), 
  BIC = sapply(m_Scha_rebased_TA366$models, BIC))

print(aic_bic_summary_Scha_rebased_TA366)

# 2.2. Comparison at hazard scale (blended vs updated vs TA366)
...
hazplot_Pem_comparison <- plot_hazard_comparison(
  h_Pem_blended, haz_Pem_7y, h_TA366, 
  rebased_time_TA366, t1, t2, 'Pembrolizumab')

hazplot_Ipi_comparison <- plot_hazard_comparison(
  h_Ipi_blended, haz_Ipi_7y, h_TA366, 
  rebased_time_TA366, t1, t2, 'Ipilimumab')

hazplot_comparison <-  ggarrange(hazplot_Pem_comparison, hazplot_Ipi_comparison)

print(hazplot_comparison)

# 2.3. Comparison at survival scale (blended vs updated vs TA366)
...
survplot_comparison <- plot_survival_comparison(
  S_Pem_blended, S_Ipi_blended, 
  km_Pem_7y, km_Ipi_7y, 
  S_Pem_TA366, S_Ipi_TA366, 
  rebased_time_TA366 ,t_seq)

print(survplot_comparison)

# 3. Comparison of 7-year RMST and incremental RMST (blended vs updated vs TA366)
# Blended model
...
# 4. Comparison of HR (blended vs updated vs TA366)
...
haz_Pem_7y_hr <- muhaz(OS.Pem.7y$Time, OS.Pem.7y$Event, bw.smooth=24, max.time=84, n.est.grid=841)
haz_Ipi_7y_hr <- muhaz(OS.Ipi.7y$Time, OS.Ipi.7y$Event, bw.smooth=24, max.time=84, n.est.grid=841)
...
HR_smoothed_df <- data.frame(
  ...
  HR = haz_Pem_7y_hr$haz.est / haz_Ipi_7y_hr$haz.est,
...
HRplot_comparison <- ggplot() + 
  ...
  geom_ribbon(data = HR_smoothed_df, aes(x = time, ymin = lower, ymax = upper), fill = "grey", alpha = 0.2) +
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "Smoothed HR of updated data" = "black",
      "Piecewise method in TA366 base case" = "#4DAF4A"
    )
  ) +
  theme_classic() + 
  theme(text = element_text(size = 12), legend.position = c(0.75, 0.85), legend.key.width = unit(1, 'cm')) +
  labs(x = 'Time (months)', y = 'HR')

print(HRplot_comparison)

# Save output
if (SAVE == T) {
  # Save AIC-BIC table for TA366 model fit
  write.csv(
    aic_bic_summary_Scha_rebased_TA366, 
    'tables/model selection/aic_bic_TA366_selection.csv', 
    row.names = T)
  # Save hazard/survival/HR figures and RMST tables
